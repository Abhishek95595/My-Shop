...
import { ShoppingBag, Trash2, ArrowRight, Sparkles, UserCheck, Store } from 'lucide-react';

export default function ShortlistPage() {
  const { user, isAuthenticated, isLoading, openLoginModal } = useAuth();
  const { shortlistProducts, clearShortlist } = useSavedItems();
      ...
      <div className="max-w-xl mx-auto px-4 py-16 text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-charcoal-600 font-sans leading-relaxed">
            Please sign in with your mock Google account to manage your shortlisted jewellery for your showroom visit.
          </p>
        </div>
        <div>
          <button
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8">
      ...
      {shortlistProducts.length > 0 && (
        <div className="bg-cream-50 border border-gold-200/90 rounded-2xl p-4 flex items-start gap-3 text-xs text-charcoal-700 font-sans">
          ...
          <p>
            Your Buying Shortlist is kept ready on this device for your store visit. You can inquire about any piece directly via WhatsApp or call before visiting.
          </p>
        </div>
      )}
